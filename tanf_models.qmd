...
sipp_all_goals <- read_csv("sipp_all_goals.csv")
...
```{r}
sipp_goal2 <- sipp_all_goals |>
  filter(in_goal2 == 1) |>
  group_by(household_id, state, year, goal2, pct_unobligated, party) |>
  summarize(
    total_tanf = sum(tanf_amt_received, na.rm = TRUE),
    job_prep = max(received_job_training, na.rm = TRUE)
    ) |>
  distinct()
```
...
# line graph - goal 2 achievement rate 2017-2021
# grouped by state
# there is variation from year-to-year
sipp_goal2 |>
  group_by(state, year) |>
  summarize(goal2_rate = mean(goal2)) |>
  ggplot(aes(x = year, group = state, y = goal2_rate)) +
  geom_line(alpha = 0.5)

sipp_goal2 |>
  group_by(state, year, pct_unobligated, party) |>
  summarize(goal2_rate = mean(goal2)) |>
...
$$
X_{ij1} = \text{whether householder i in state j is married at end of reference frame} \\
X_{ij2} = \text{whether householder i in state j participated in job preparation} \\
X_{ij3} = \text{whether householder i in state j is employed at end of reference frame} \\
X_{ij4} = \text{amount of TANF funds received by household i in state j during reference period} \\
X_{j1} = \text{percent of unobligated funds for state j}
$$

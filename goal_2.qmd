...
-   Percent goal 2 success for each state averages about 0.28 with a standard deviation of 0.21. Prior model for $\beta_{0c}$: $\beta_{0c} \sim N(0.28, 0.21^2)$
...
-   we'll use weakly informative priors for standard deviation parameters
    ...
    goal_2_model_1_prior <- stan_glmer(
      pct_goal2 ~ pct_unobligated + (1 | state),
      data = goal2, family = gaussian,
      prior_intercept = normal(28, 21),
      prior = normal(1, 2),
      prior_aux = exponential(1, autoscale = TRUE), 
      prior_covariance = decov(reg = 1, conc = 1, shape = 1, scale = 1),
      chains = 10, iter = 10000, seed = 84735,
      prior_PD = TRUE
    )
...
goal2 |>
  add_predicted_draws(goal_2_model_1_prior, n = 100) %>%

...
ggplot(us_hexbin2) +
  geom_sf(aes(fill = mean_direct_assistance), color = "white", linewidth = 1) +
  geom_sf_text(aes(label = iso3166_2), color = "black", family = "Lexend") +
  theme_void() +
  scale_fill_distiller(direction = 1) +
  theme(text = element_text(family = "Lexend"),
  ...
  labs(fill = "Mean Percent Direct Assistance",
       title = "Percent TANF Funds Spent on Direct Assistance by State")
...
expenditures_bar4 <- expenditures |>
  ...
  select(state, funds_awarded, ccdf_discretionary, ssbg_block_grant, adjusted_award, carryover, basic_assistance, services, total_expenditures, fed_unliquidated, unobligated_balance)

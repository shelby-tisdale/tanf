...
goal_3_dev <- sipp_participants |>
  # filters for women between ages 
  filter(age >= 15 & age <= 50 & marital_status != 1 & marital_status != 2 & sex == 2) |>
  ...
  select(sample_unit_id, person_num, min_date, max_date, num_children, interval, birth_year) |>
  distinct() |>
...
hold <- sipp_participants |>
  select(sample_unit_id, person_num, month, year, is_parent, num_children, age, childbearing_union, marital_status, unmarried_partner_num,
         child1_num:child12_num) |>
  right_join(goal_3_dev) |>

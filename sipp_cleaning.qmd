...
sipp$hh_relationship

# load new data
sipp <- read.csv("sipp.csv")
```
...
parent_households <- sipp |>
  ...
  summarize(
    ...
    # each child has a reference parent
    # so this indicates whether or not a child is in the household
    ref_par = sum(reference_par == 1 & age >= 15, na.rm = TRUE)/12,
    .groups = "drop"
    ) |>
  # indicator for whether parent in household
  mutate(hh_has_parent = if_else(ref_par > 0, 1, 0)) |>
...
households_with_children <- sipp |>
  ...
  summarize(hh_children_count = sum(age <= 18), .groups = "drop") |>

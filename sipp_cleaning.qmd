...
parent_child_lookup <- sipp_participants |>
  ...
  select(person_num, sample_unit_id, starts_with("child"), starts_with("rrel")) |>
  ...
  distinct() |>
  mutate(hh_member_number = extract_numeric(hh_member_number),
         hh_member_number2 = extract_numeric(hh_member_number2)) |>
  filter(hh_member_number == hh_member_number2) |>
  distinct()

parent_child_lookup <- parent_child_lookup |>
  mutate(parent_number = person_num) |>
  select(sample_unit_id, parent_number, child_person_number, relationship) |>
  (distinct)()


parent_child_lookup <- sipp_participants |>
  filter(reference_par == 1) |>
  select(person_num, sample_unit_id, starts_with("rrel")) |>
